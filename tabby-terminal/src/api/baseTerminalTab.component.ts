...
export class BaseTerminalTabComponent<P extends BaseTerminalProfile> extends BaseTabComponent implements OnInit, OnDestroy {
    ...
    ngOnInit (): void {
        ...
        setTimeout(async () => {
            // Always attach frontend immediately, regardless of focus state
            // This ensures SSH connections start without requiring a click
            await this.frontend?.attach(this.content.nativeElement, this.profile)
            this.frontend?.configure(this.profile)

            // Force trigger frontendReady if it hasn't been triggered yet
            // This is a fallback in case the resize event doesn't fire
            if (!this.frontendIsReady && this.frontend) {
                // The resize event should have been emitted by attach(), but if not,
                // we need to ensure frontendReady is triggered
                // This is handled by the forced resize.next() in xtermFrontend.attach()
            }
        })
    ...
    async getCompletion (input: string): Promise<string[]> {
        ...
        const commonCommands = [
            'ls', 'cd', 'pwd', 'git', 'docker', 'npm', 'yarn', 'node', 'python', 'ssh',
            'cat', 'grep', 'find', 'mkdir', 'rm', 'cp', 'mv', 'chmod', 'chown', 'ps',
            'kill', 'top', 'htop', 'df', 'du', 'tar', 'zip', 'unzip', 'curl', 'wget',
            'vim', 'nano', 'emacs', 'less', 'more', 'tail', 'head', 'history', 'clear', 'exit',
        ]
        ...
        return commonCommands.filter(cmd => cmd.startsWith(input))
    }
}
