...
export class XTermFrontend extends Frontend {
    ...
    async attach (host: HTMLElement, profile: BaseTerminalProfile): Promise<void> {
        ...
        // Force emit a resize event to ensure frontendReady is triggered
        // This is critical for SSH connections to start automatically
        // Use actual dimensions if available, otherwise use sensible defaults
        const cols = this.xterm.cols > 0 ? this.xterm.cols : 80
        const rows = this.xterm.rows > 0 ? this.xterm.rows : 24

        // Always emit the resize event, even if resizeHandler already did
        // This ensures frontendReady is triggered regardless of element visibility
        this.resize.next({ rows, columns: cols })
