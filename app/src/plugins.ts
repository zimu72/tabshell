...
const builtinPluginsPath = process.env.TABSHELL_DEV ? path.dirname(remote.app.getAppPath()) : path.join((process as any).resourcesPath, 'builtin-plugins')
...
export function initModuleLookup (userPluginsPath: string): void {
    ...
    if (process.env.TABSHELL_PLUGINS) {
        process.env.TABSHELL_PLUGINS.split(':').map(x => paths.push(normalizePath(x)))
    }
