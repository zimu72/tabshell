...
export default options => {
    const sourceMapOptions = {
        ...
        filename: '[file].map',
        moduleFilenameTemplate: `webpack-tabshell-${options.name}:///[resource-path]`,
    }
    ...
    if ((process.platform === 'win32' || process.platform === 'linux') && process.env.TABSHELL_DEV) {
        devtoolPlugin = wp.EvalSourceMapDevToolPlugin
    }

    const isDev = !!process.env.TABSHELL_DEV
    const config = {
        target: 'node',
        entry: 'src/index.ts',
