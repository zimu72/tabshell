...
export let version = '1.0.0-dev'
try {
    version = childProcess.execSync('git describe --tags', { encoding:'utf-8' })
    version = version.substring(1).trim()
    version = version.replace('-', '-c')
} catch (e) {
    version = JSON.parse(fs.readFileSync(path.resolve(__dirname, '../package.json'))).version ?? version
    console.warn('Could not determine version from git, using default', version)
}
