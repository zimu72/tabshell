...
TabShell is an Electron app, with the frontend written in Typescript with the help of Angular framework. It's built using Webpack.

TabShell is based on [Tabby](https://github.com/Eugeny/tabby) by Eugene Pankov.
...
First, from within the `tabshell` directory install the dependencies via yarn:
...
The app will load all plugins from the source checkout in the dev mode, from the user's plugins directory at all times (click `Open Plugins Directory` under `Settings` > `Plugins`) and from the directory specified by the `TABSHELL_PLUGINS` environment var.

Only modules whose `package.json` file contains a `tabby-plugin` keyword will be loaded.

If you're currently in your plugin's directory, start TabShell as `TABSHELL_PLUGINS=$(pwd) tabshell --debug`
