    ...
    async getItems (tab: BaseTabComponent, tabHeader?: boolean): Promise<MenuItemOptions[]> {
        ...
        if (tabHeader) {
            ...
            items = [
                ...items,
                {
                    label: this.app.tileMode ? this.translate.instant('Tab Mode') : this.translate.instant('Tile Mode'),
                    commandLabel: this.translate.instant('Toggle tile mode'),
                    click: () => this.app.toggleTileMode(),
                },
                {
