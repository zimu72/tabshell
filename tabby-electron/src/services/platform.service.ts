...
export class ElectronPlatformService extends PlatformService {
    ...
    async getFileZillaPath (): Promise<string|null> {
        ...
        if (this.hostApp.platform === Platform.Windows) {
            ...
            for (const base of baseDirs) {
                candidates.push(path.join(base, 'FileZilla FTP Client', 'filezilla.exe'))
            }

            const pathEntries = (process.env.PATH ?? '').split(path.delimiter).filter(Boolean)
            for (const entry of pathEntries) {
                candidates.push(path.join(entry, 'filezilla.exe'))
                candidates.push(path.join(entry, 'tabftp.exe'))
            }
        } else if (this.hostApp.platform === Platform.macOS) {
            candidates.push('/Applications/FileZilla.app/Contents/MacOS/filezilla')
            candidates.push(path.join(os.homedir(), 'Applications', 'FileZilla.app', 'Contents', 'MacOS', 'filezilla'))
        } else {
            candidates.push(
                '/usr/bin/tabftp',
            ...
            const pathEntries = (process.env.PATH ?? '').split(path.delimiter).filter(Boolean)
            for (const entry of pathEntries) {
                candidates.push(path.join(entry, 'tabftp'))
                candidates.push(path.join(entry, 'filezilla'))
            }
        }
